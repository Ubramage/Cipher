namespace SpriteKind {
    export const observer = SpriteKind.create()
}
...
scene.onOverlapTile(SpriteKind.Player, sprites.dungeon.chestOpen, function (sprite, location) {
    mySprite.sayText("go back", 500, true)
})
...
game.onUpdate(function () {
    if (mySprite.tilemapLocation().row == 13) {
        game.setGameOverEffect(false, effects.clouds)
        game.gameOver(false)
    } else if (mySprite.tileKindAt(TileDirection.Center, sprites.dungeon.collectibleInsignia)) {
        game.gameOver(true)
    }
})
